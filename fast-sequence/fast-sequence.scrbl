...
This will run fast, but the code was nicer in the previous solution. Here we lose abstraction of working with sequences,
and we can no longer define a reusable sequence.
...
When used directly in a @racket[for] loop, returns a fast sequence form whose elements are results of evaluating the last
@racket[body] on each iteration of the loop. Using outside a @racket[for] loop is forbidden; in that case, it raises an error.

The @racket[do/sequence] form provides two kinds of @italic{clauses}: a @racket[binding-clause] and a @racket[when-clause],
which specify the number of elements of the sequence. These clauses have the same meaning as the corresponding clauses
of the @racket[for] loop. Other kinds of @racket[for] clauses (e.g., @racket[#:break]) are not supported.
