    ...
    (syntax-parse stx
      [[(id:id ...) (_ (b-clause:bind-clause ...+) seq-expr:expr)] #:cut
       ...
       (with-syntax* ([(loop-id* ...) (generate-temporaries #'(eb.loop-id ...))]
                      ...
                      [(i-loop-arg*** ...) (generate-temporaries #'(eb-i.loop-arg ...))]
                      ...
                      [(i-pre-guard** i-post-guard**) (generate-temporaries #'(i-pre-guard** i-post-guard**))]
         ...
         (for-clause-syntax-protect
          #'[(id ...)
             (:do-in
                ...
                (let ([loop-arg-id (lambda (eb.loop-id ...) (lambda (eb.inner-id ... ...) loop-arg**))] ...)
                    ...
                    (lambda (eb.inner-id ... ...)
                      (lambda (eb-i.outer-id ... ...)
                        (lambda (eb-i.loop-id ...)
                          (cond
                            [eb-i.pos-guard
                             (let-values ([(i-inner-id* ...) eb-i.inner-rhs] ...
                                          [(i-pre-guard**) (lambda (eb-i.inner-id ... ...) eb-i.pre-guard)]
                                          [(i-post-guard**) (lambda (eb-i.inner-id ... ...) eb-i.post-guard)]
                                          [(i-loop-arg***) (lambda (eb-i.inner-id ... ...) i-loop-arg*)] ...
                                          [(id*) (lambda (eb-i.inner-id ... ...) id)] ...)
                               (if (and (i-pre-guard** i-inner-id* ... ...)
                                        i-post-guard*)
                                   ;; Case 1
                                   (begin
                                     #;(println (list "values"))
                                     (values (i-loop-arg*** i-inner-id* ... ...) ... eb.loop-id ... #t
                                             (id* i-inner-id* ... ...) ... eb.inner-id ... ... eb-i.outer-id ... ... #t
                                             post-guard* (i-post-guard** i-inner-id* ... ...)))
                                   (begin
                                     #;(println (list "loop-without" eb.loop-id ... post-guard*))
                      ...
                      [pos-guard-id*
                       (let-values ([(eb.inner-id ...) eb.inner-rhs] ...)
                         (if (and eb.pre-guard
                                  post-guard*)
                             ;; Case 2
                             (let-values ([(loop-arg***  ...) (values loop-arg* ...)]
                                          [(eb-i.outer-id ...) eb-i.outer-rhs] ...)
                               i-outer-check*
                               #;(println (list "loop-with" eb.inner-id ... ...
                                              eb-i.outer-id ... ... eb.post-guard #t))
                  ...
                  (cond
                    [ids-ok
                     (begin
                       #;(println (list "loop-with" eb.inner-id ... ... ...
                                      i-outer-id* ... (and eb.post-guard ...) #t))
              ...
              ;; post guard
              (begin
                #;(println (list "loop" loop-id* ... i-loop-id** ... inner-id* ... i-outer-id* ... ids-ok post-guard* i-post-guard*))
                ok)
              ;; loop args
